 ...
 * "Channel Modeling and Capacity Analysis for Electromagnetic Wireless Nanonetworks in the
 * Terahertz Band"
 *
...
class THzChannel : public Channel
{
  ...
  public:
     ...
     *
     * in this function a NoiseEntry will be updated, which recoreds the signel strength a node will
     * receive and how long will the signal exist, this information will be used by terahertz
     * physical layer to calculate SINR. Also in this function, it checks the antenna mode for
     * sending node and receiving node, by this process it checking the receiver's orientation and
     * thus the total antenna gain between transmitter and receiver. Note that the sending node is
     * not necessarily to be the transmitter, e.g., node sending CTS or ACK packet is actually the
     * receiver.
     */
  ...
  private:
     ...
     *
     * Upon receive packet from terahertz channel, the node updates the noiseEntry for terahertz
     * physical layer SINR calculation.
     */
     ...
     * \param ne the noise entry.
     *
     * Note that if concurrent transmissions end at the same time, some of them can be missed from
     * SINR calculation, so delete a noise entry a few seconds later.
     */

...
THzEnergyModel::HarvestEnergy(void)
{
    ...
    m_energyUpdateEvent = Simulator::Schedule(m_energyUpdateInterval, &THzEnergyModel::HarvestEnergy, this);
    NS_LOG_DEBUG("node id: " << m_node->GetId()
                             << " remaining energy: " << m_remainingEnergy
                             << " now: " << Simulator::Now());
}

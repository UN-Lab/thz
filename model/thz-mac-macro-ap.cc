...
{
  static TypeId tid = TypeId ("ns3::THzMacMacroAp")
    .SetParent<THzMac> ()
...
THzMacMacroAp::SendCta1 ()
{
  ...
  Time dataTimeout = GetCtrlDuration (THZ_PKT_TYPE_CTS) + m_tProp + GetSifs() + GetMaxBackoff() + m_tProp + NanoSeconds(1);
  m_dataTimeoutEvent = Simulator::Schedule (dataTimeout ,&THzMacMacroAp::DataTimeout, this);

  NS_LOG_UNCOND (Simulator::Now() << " - AP - CTS generated" << " at node " << m_nodeId << " Data T/O in: " << dataTimeout);
...
THzMacMacroAp::SendCta3 ()
{
  ...
  Time waitTime = GetCtrlDuration (THZ_PKT_TYPE_CTA) + m_tProp + GetSifs() + GetMaxBackoff() + m_tProp + GetCtrlDuration (THZ_PKT_TYPE_RTS) + NanoSeconds(1);
...
THzMacMacroAp::WaitTimeExpired ()  // Wait Time will always expire. Check how many RTS were received
{
  ...
  if (m_rtsList.empty())
    {
      NS_LOG_UNCOND (Simulator::Now() << " - AP - ---------- No RTS received, turning to next sector at " << m_angle + m_beamwidth << " ----------");
      TurnRxAntenna();
      return;
    }
  ...
  NS_LOG_UNCOND (Simulator::Now() << " - AP - Wait Time Expired. Received " << m_rtsList.size() << " RTSs.");
...
THzMacMacroAp::SectorTimeout () 
{
  if (!m_ackList.empty())
    {
      NS_LOG_UNCOND (Simulator::Now() << " - AP - ---------- SECTOR TIMEOUT. Sending " << m_ackList.size() << " ACKs ----------");
      SendAck();
    }
  else
    {
      NS_LOG_UNCOND (Simulator::Now() << " - AP - ---------- SECTOR TIMEOUT. No correct data received. Turning to next sector at " << m_angle + m_beamwidth<< " ----------");
      TurnRxAntenna();
    }
}
...
{
  if (power > csth_64QAM) { return 14; }
  if (power > csth_16QAM) { return 13; }
  if (power > csth_8PSK)  { return 12; }
  if (power > csth_QPSK)  { return 11; }
  if (power > csth_BPSK)  { return 10; }
  return 0;
}
...
THzMacMacroAp::SendPacketDone (Ptr<Packet> packet)
{
  ...
  switch (header.GetType ())
    ...
    case THZ_PKT_TYPE_ACK:
      if (m_ackList.empty())
        {
          NS_LOG_UNCOND (Simulator::Now() << " - AP - ---------- ACK sent. Turning to next sector at " << m_angle + m_beamwidth<< " ----------");
...
{
  if (m_dataTimeoutEvent.IsRunning()) { m_dataTimeoutEvent.Cancel(); }

  THzMacHeader header;
  packet->PeekHeader (header);
  NS_LOG_FUNCTION ("at node " << m_nodeId << " from " << header.GetSource () << " now " << Simulator::Now () << " state: " << StateToString (m_state));
  switch (m_state)
    {
    case WAIT_TX:
    case RX:
  ...
  for (it4 = m_nodeMap.begin(); it4 != m_nodeMap.end(); it4++)
    {
      ...
      i++;
      Simulator::Schedule ((i-1) * (GetCtrlDuration (THZ_PKT_TYPE_CTS) + NanoSeconds(1)), &THzMacMacroAp::SendFeedbackCTA, this, bestSector, node_mac);
    }
  Simulator::Schedule (i * (GetCtrlDuration (THZ_PKT_TYPE_CTS) + NanoSeconds(1)), &THzMacMacroAp::TurnRxAntenna, this);
}

...
THzPhyMacro::SendPacket(Ptr<Packet> packet, bool rate, uint16_t mcs)
{
    ...
    NS_LOG_DEBUG("Tx will finish at " << (Simulator::Now() + txDuration).GetPicoSeconds()
                                      << "(ps) txPower=" << m_txPower << " dBm");
...
{
    NS_LOG_FUNCTION("at node " << m_device->GetNode()->GetId() << " rxPower " << rxPower << " dBm"
                               << "busyEnd" << m_csBusyEnd << " state " << (m_state));
    ...
    if (rxPower < m_csTh)
    {
        ...
        /*if (rxPower > m_csTh - 10){
          NS_LOG_UNCOND(m_device->GetNode ()->GetId () << ": Not enough power. rxPower: " << rxPower
        << ", threshold: " << m_csTh);
        }*/
        return;
    }
...
{
    NS_LOG_FUNCTION("at node " << m_device->GetNode()->GetId() << "csBusy " << m_csBusyEnd
                               << " now " << Simulator::Now() << " state " << (m_state));
    ...
    if (packet == m_pktRx)
    {
        ...
        // ADD: CHANGE STATUS
        m_state = IDLE;
        // NS_LOG_UNCOND(m_device->GetNode ()->GetId () << ": SINR = " << sinrDb << " dB; SINR TH =
        // " << m_sinrTh << " dB");
        if (sinrDb > m_sinrTh)
        {
            m_state = IDLE;

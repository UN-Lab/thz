...
THzPhyNano::SendPacket(Ptr<Packet> packet, bool rate, uint16_t mcs)
{
    ...
    if (m_ongoingTx.size() != 0 ||
        m_ongoingRx.size() != 0) // the m_ongoingTx is not empty and less than Ts/m_pulse
    {
        NS_LOG_INFO("Size of the transmission list:"
                    << m_ongoingTx.size() << "Size of the receive list:" << m_ongoingRx.size());
        ...
        for (; itr != m_ongoingRx.end(); ++itr)
        {
            ...
            else
            {
                ...
                NS_LOG_INFO("nowD:" << nowD << " TsD" << TsD << " fmod"
                                    << fmod((now - itr->m_rxStart).ToDouble(Time::FS), TsD));
                nextPulse[txCount] = (nowD + TsD - mod);
                NS_LOG_INFO("now:" << now << "The next pulse of reception will appear at:"
                                   << FemtoSeconds(nextPulse[txCount]));
                txCount++;
            }
        }
        ...
        NS_LOG_INFO("Size of nextPulse: " << sizeof(nextPulse) / sizeof(nextPulse[0])
                                          << "NextPulse[0] " << FemtoSeconds(nextPulse[0]));
        for (int j = 1; j <= size; j++)
        {
            if (nextPulse[j - 1] != (nowD + TsD) && nextPulse[j - 1] + 2 * m_pulseDuration.ToDouble(Time::FS) <= nextPulse[j])
            {
                ...
                Simulator::Schedule(ot.m_txStart - Simulator::Now(),
                                    &THzPhyNano::ScheduleSendPacket,
                                    this,
                                    packet,
                                    txDuration);
                ...
                NS_LOG_DEBUG("Tx interleaved and will finish at "
                             << (ot.m_txStart + txDuration).GetFemtoSeconds() << "fs txPower"
                             << m_txPower);
                Simulator::Schedule((ot.m_txStart - Simulator::Now()) + txDuration,
                                    &THzPhyNano::DeleteOngoingTx,
                                    this,
                                    ot);
                return true;
            }
    ...
    NS_LOG_DEBUG("Tx not interleaved and will finish at "
                 << (Simulator::Now() + txDuration).GetFemtoSeconds() << "fs txPower" << m_txPower);

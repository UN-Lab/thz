...
THzUdpClient::StartApplication(void)
{
    ...
    /*
      Notify application when new data is available to be read. This callback is intended to notify a
      socket that would have been blocked in a blocking socket model that data is available to be read.
    */
...
THzUdpClient::Send(void)
{
    ...
    if ((m_socket->Send(p)) >= 0) // Send data (or dummy data) to the remote host
    {
        ...
        x->SetAttribute("Mean", DoubleValue(m_mean));
        // x->SetAttribute("Bound", DoubleValue (std::max(1000.0, m_mean*3)));
        // Be careful: bounding shifts the mean to a lower value!
        m_delay = MicroSeconds(x->GetValue());
        ...
        NS_LOG_INFO("from node " << GetNode()->GetId() << " TraceDelay TX " << m_size
                                 << " bytes to " << peerAddressStringStream.str() << " Uid: "
                                 << p->GetUid() << " Time: " << (Simulator::Now()).GetSeconds());
    }
    else
    {
